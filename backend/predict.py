...
from config import MODEL_PATH, CONFIDENCE_THRESHOLD, GEMINI_API_KEY, OPENROUTER_API_KEY, HF_API_KEY, DISABLE_GRADCAM, FORCE_LOCAL_INFERENCE
...
class InferenceEngine:
    ...
    def _generate_gradcam_safe(self, image: Image.Image, disease_name: str) -> Tuple[str, str]:
        """Safely generates Grad-CAM heatmap and overlay base64 data URL strings."""
        if DISABLE_GRADCAM:
            return "", ""
        try:
            ...
            mapped_disease = disease_name
            if disease_name == "Pleural Effusion" and "Effusion" in self.classes:
                mapped_disease = "Effusion"
            elif disease_name == "Pulmonary Edema" and "Edema" in self.classes:
                mapped_disease = "Edema"
            elif disease_name == "Lung Mass/Nodule" and "Mass" in self.classes:
                mapped_disease = "Mass"
            
            def normalize_name(name):
                return name.lower().replace("-", "").replace(" ", "").replace("tuberclosis", "tuberculosis").replace("turberculosis", "tuberculosis")
                
            norm_classes = [normalize_name(c) for c in self.classes]
            norm_disease = normalize_name(mapped_disease)
            
            if norm_disease not in norm_classes:
                logger.warn(f"Disease '{disease_name}' not in local classes {self.classes}. Skipping Grad-CAM.")
                return "", ""
                
            class_idx = norm_classes.index(norm_disease)
            ...
            # Generate
            heatmap = self.grad_cam.generate_heatmap(tensor, class_idx)
            heatmap_b64, overlay_b64 = overlay_heatmap(heatmap, image)
            return heatmap_b64, overlay_b64
        except Exception as e:
            logger.error(f"Grad-CAM safe generation failed: {e}")
            return "", ""
    ...
    def predict(self, image: Image.Image) -> Dict[str, Any]:
        ...
        if GEMINI_API_KEY and not FORCE_LOCAL_INFERENCE:
            gemini_models = ["gemini-3.6-flash", "gemini-2.5-flash", "gemini-2.0-flash", "gemini-1.5-flash", "gemini-3.1-flash-lite", "gemini-3.5-flash-lite"]
        ...
        if not gemini_probs and HF_API_KEY and not FORCE_LOCAL_INFERENCE:
            try:
                # Resize image to max 512x512 before encoding — full-res X-rays cause silent API failures
                api_image = image.convert("RGB")
                api_image.thumbnail((512, 512), Image.LANCZOS)
        ...
        if gemini_probs:
            try:
                ...
                heatmap_b64, overlay_b64 = self._generate_gradcam_safe(image, primary["disease"])
